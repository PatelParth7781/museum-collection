...
const statusColors: Record<string, string> = {
  active: 'bg-green-100 text-green-700', on_loan: 'bg-blue-100 text-blue-700',
  in_conservation: 'bg-yellow-100 text-yellow-700', archived: 'bg-stone-200 text-stone-600',
};
    ...
    <DashboardLayout title="Artifact Management">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div className="flex gap-3 flex-1 flex-wrap">
          ...
          <SelectField value={conditionFilter} onChange={(e) => { setConditionFilter(e.target.value); setPage(1); }} className="w-auto">
        ...
        <Link to="/admin/artifacts/new" className="btn-primary whitespace-nowrap">
      ...
      {loading ? (
        ...
        <div className="card"><EmptyState icon={<Package size={48} strokeWidth={1.5} />} title="No artifacts found" message="Try adjusting your filters or add a new artifact." action={<Link to="/admin/artifacts/new" className="btn-primary"><Plus size={16} /> Add Artifact</Link>} /></div>
      ) : (
          ...
          <div className="overflow-x-auto scrollbar-thin">
            <table className="w-full">
              <thead className="bg-stone-50 border-b border-stone-200">
                <tr>
                  {['Image', 'Accession #', 'Name', 'Category', 'Period', 'Condition', 'Location', 'Status', 'Actions'].map((h) => (
                  ...
                  <tr key={a.id} className="hover:bg-stone-50 transition-colors">
                    <td className="px-4 py-3">
                      <div className="w-12 h-12 rounded-lg overflow-hidden bg-stone-100 shrink-0">
                        {a.artifact_images?.[0] ? (
                          <img src={a.artifact_images[0].image_url} alt={a.name} className="w-full h-full object-cover" />
                        ) : (
                    ...
                    <td className="px-4 py-3 text-sm text-stone-600">{a.historical_period?.name ?? '—'}</td>
                    <td className="px-4 py-3"><span className={`badge ${conditionColors[a.condition]}`}>{a.condition}</span></td>
                    <td className="px-4 py-3 text-sm text-stone-600 whitespace-nowrap">{a.current_location ? `${a.current_location.building} — ${a.current_location.gallery}` : '—'}</td>
                    <td className="px-4 py-3"><span className={`badge ${statusColors[a.status]} capitalize`}>{a.status.replace('_', ' ')}</span></td>
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={() => handleArchive(a.id)} className="p-1.5 rounded-lg text-stone-400 hover:text-yellow-600 hover:bg-yellow-50" aria-label="Archive"><Archive size={16} /></button>
                        <button onClick={() => setDeleteTarget(a.id)} className="p-1.5 rounded-lg text-stone-400 hover:text-red-600 hover:bg-red-50" aria-label="Delete"><Trash2 size={16} /></button>
                      </div>
                    </td>
                  </tr>
                ))}
      ...
      <ConfirmDialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} title="Delete Artifact" message="Are you sure you want to delete this artifact? This will also remove all related images, provenance, and conservation records. This action cannot be undone." confirmLabel="Delete" danger />
    </DashboardLayout>
  );
}
