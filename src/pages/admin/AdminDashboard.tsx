      ...
      try {
        ...
        const [mostReviewedRes, highestRatedRes] = await Promise.all([
          supabase.from('reviews')
            .select('artifact:artifacts(name)')
            .eq('status', 'published'),
          supabase.from('reviews')
            .select('artifact:artifacts(name), rating')
            .eq('status', 'published'),
        ]);
        ...
        // User growth chart — cumulative registrations over last 6 months
