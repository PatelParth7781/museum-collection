...
export default function CuratorExhibitions() {
  ...
  const openCreate = () => { setEditTarget(null); setForm({ name: '', description: '', start_date: '', end_date: '', location_id: '', status: 'upcoming', cover_image_url: '' }); setModalOpen(true); };
  const openEdit = (exh: Exhibition) => { setEditTarget(exh); setForm({ name: exh.name, description: exh.description, start_date: exh.start_date ?? '', end_date: exh.end_date ?? '', location_id: exh.location_id ?? '', status: exh.status, cover_image_url: exh.cover_image_url ?? '' }); setModalOpen(true); };
    ...
    <DashboardLayout title="Exhibition Management">
      ...
      <Modal open={modalOpen} onClose={() => setModalOpen(false)} title={editTarget ? 'Edit Exhibition' : 'Create Exhibition'} size="lg">
        <form onSubmit={handleSubmit} className="space-y-4">
          <FormField label="Exhibition Name" required><TextInput value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="e.g. Echoes of the Ancients" /></FormField>
          ...
          <div className="grid grid-cols-2 gap-4">
            <FormField label="Location"><SelectField value={form.location_id} onChange={(e) => setForm({ ...form, location_id: e.target.value })}><option value="">Select location...</option>{locations.map((l) => <option key={l.id} value={l.id}>{l.building} — {l.gallery}</option>)}</SelectField></FormField>
            <FormField label="Status"><SelectField value={form.status} onChange={(e) => setForm({ ...form, status: e.target.value })}><option value="upcoming">Upcoming</option><option value="active">Active</option><option value="ended">Ended</option><option value="cancelled">Cancelled</option></SelectField></FormField>
          </div>
          <FormField label="Cover Image URL"><TextInput value={form.cover_image_url} onChange={(e) => setForm({ ...form, cover_image_url: e.target.value })} placeholder="https://..." /></FormField>
          <div className="flex justify-end gap-3 pt-2"><button type="button" onClick={() => setModalOpen(false)} className="btn-secondary">Cancel</button><button type="submit" disabled={formLoading} className="btn-primary">{formLoading ? 'Saving...' : editTarget ? 'Update' : 'Create'}</button></div>
        </form>
      </Modal>
      ...
      <ConfirmDialog open={!!deleteTarget} onClose={() => setDeleteTarget(null)} onConfirm={handleDelete} title="Delete Exhibition" message="Are you sure? This cannot be undone." confirmLabel="Delete" danger />
    </DashboardLayout>
  );
}
